...
import { aggregateReactions, artifactMetaByAttachmentIds } from "../core.js";
...
export async function attachMentions(msgs: (typeof schema.messages.$inferSelect)[]) {
  ...
  const ameta = await artifactMetaByAttachmentIds(atts.map((a) => a.id));
  ...
  return msgs.map((m) => ({
    ...
    attachments: atts.filter((a) => a.messageId === m.id).map((a) => {
      const am = ameta.get(a.id);
